...
SELECT menu_name, menu_price, menu_price * 0.1, (menu_price + menu_price*0.1)
FROM tbl_menu;

# ===============================================
# ResultSet : SELECT 조회 결과행의 집합
# 컬럼명 as 별칭 : ResultSet에 표기되는 컬럼명에 별칭(alias) 부여 가능
SELECT menu_name AS 메뉴명, menu_price AS "메뉴 가격",
       menu_price * 0.1 부가세, (menu_price + menu_price*0.1) "부가세 포함 가격"
FROM tbl_menu;
# ===============================================

# 산술 연산
select 10 + 3, 10 - 3, 10 * 3, 10 / 3, 10 div 3, 10 % 3, 10 mod 3;

# 문자열 연결 처리(이어쓰기)
# concat("abc" + "def") == "abcdef"
# 메뉴 테이블에서 메뉴명, 메뉴가격(원) 조회
SELECT menu_name as 메뉴명, concat('금', menu_price, '원') as "메뉴 가격(원)"
FROM tbl_menu;
